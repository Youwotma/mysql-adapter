...
exports.initialize = function initializeSchema(schema, callback) {
    ...
    // create adapter
    var adapter = schema.adapter = new MySQL(schema);

    // MySQL specific column types
    schema.constructor.registerType(function Point() {});

    //try to use the schema
    adapter.query('USE `' + schema.settings.database + '`', callback);
...
function MySQL(schema) {
    this.name = 'mysql-pooled';
    this.schema = schema;
    this._models = {};
    this.pool = MySQL._createPool(schema);
    this.maxRetries = schema.settings.maxRetries || DEFAULT_MAX_RETRIES;
    ...
    setInterval(function () {
        var r = _.clone(self.audit), keys = Object.keys(r); 
        self.audit = {}; 
        if (keys.length) {
            console.log('SQL EXECUTION TIMES:', process.pid, (new Date()).getTime(), keys.map(function (k) { var a = r[k]; return (a[1] - a[0]) + '|' + (a[2] - a[1]);}).join(',')); 
        }
    }, 10000);    
}

MySQL._createPool = function (schema) {
    ...
    return mysql.createPool({
        ...
        database: s.database,
        debug: s.debug,
        connectionLimit: s.poolSize || DEFAULT_POOL_SIZE,
        createConnection: function createAndBindMySQLConnection (opts) {
            var client = mysql.createConnection(opts);
            client.on('error', function (err) { schema.emit('error', err); });
            return client;
        }
    });
};
...
MySQL.prototype.query = function (sql, callback) {
    ...
    this._executeQuery(sql, callback, 0);
};

MySQL.prototype._executeQuery = function (sql, callback, times) {
    if (!times || typeof(times) !== 'number') times = 0;
    var ai = this.auditCnt++;
    var self = this;

    function __retryQuery (e,d) {
        if (times < self.maxRetries) {
            process.nextTick(function () {
                self._executeQuery(sql, callback, ++times);
            });
        } else {
            callback(e,d);
        }
    ...
    callback = function (e,d) {
        if (Array.isArray(self.audit[ai])) self.audit[ai].push((new Date()).getTime());
        _callback(e,d);
    };

    
    this.audit[ai] = [(new Date()).getTime()];

    this.pool.getConnection(function (connErr, conn) {
        if (connErr) {
            __retryQuery(connErr);
            return;
        }
        if (Array.isArray(self.audit[ai])) self.audit[ai].push((new Date()).getTime());
        conn.query(sql, function (err, data) {
            if (err) {
                if (err.message.match(/(^|: )unknown database/i) && times === 0) {
                    var dbName = err.message.match(/(^|: )unknown database '(.*?)'/i)[1];
                    conn.query('CREATE DATABASE ' + dbName, function (error) {
                        if (!error) {
                    ...
                    __retryQuery(err, data);
                    conn.destroy();
                }
                return callback(err);
            } 
            conn.end();
            if (typeof(self.log) === 'function') self.log(sql, Date.now());
            callback(err, data);
        });
...
MySQL.prototype.poolState = function () {
    return {
        max: this.pool.config.connectionLimit,
        size: this.pool._allConnections.length,
...
function dateToMysql(val) {
    return val.getUTCFullYear() + '-' + fillZeros(val.getUTCMonth() + 1) + '-' + fillZeros(val.getUTCDate()) + ' ' + fillZeros(val.getUTCHours()) + ':' + fillZeros(val.getUTCMinutes()) + ':' + fillZeros(val.getUTCSeconds());
...
MySQL.prototype.toDatabase = function(prop, val) {
    ...
    if (val.constructor.name === 'Object') {
        ...
        if (operator === 'between') {
            return this.toDatabase(prop, val[0]) +
                ' AND ' + this.toDatabase(prop, val[1]);
    ...
    return mysql.escape(val.toString());
};
...
MySQL.prototype.all = function all(model, filter, callback) {
    ...
    if (filter) {
        ...
        if (filter.where) {
            sql += ' ' + buildWhere(filter.where, props, self);
        }
...
MySQL.prototype.count = function count(model, callback, where) {
    ...
    this.queryOne('SELECT count(*) as cnt FROM ' + this.tableEscaped(model) + ' ' + buildWhere(where, props, self), function(err, res) {
...
MySQL.prototype.propertySettingsSQL = function(model, prop) {
    ...
    return datatype(p) + ' ' + (p.allowNull === false || p['null'] === false ? 'NOT NULL' : 'NULL');
};
...
function buildWhere(conds, props, mysql_instance) {
    var cs = [];
    Object.keys(conds).forEach(function(key) {
        var keyEscaped = '`' + key.replace(/\./g, '`.`') + '`';
        var val = mysql_instance.toDatabase(props[key], conds[key]);
