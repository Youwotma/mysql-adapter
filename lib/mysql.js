...
MySQL.prototype.all = function all(model, filter, callback) {
    ...
    if (filter) {
        ...
        if (filter.where) {
            sql += ' ' + buildWhere(filter.where, props);
        }
...
MySQL.prototype.count = function count(model, callback, where) {
    ...
    this.queryOne('SELECT count(*) as cnt FROM ' +
        this.tableEscaped(model) + ' ' + buildWhere(where, props), function (err, res) {
...
function buildWhere(conds, props) {
    var cs = [];
    Object.keys(conds).forEach(function(key) {
        var keyEscaped = '`' + key.replace(/\./g, '`.`') + '`'
        var val = MySQL.prototype.toDatabase(props[key], conds[key]);
